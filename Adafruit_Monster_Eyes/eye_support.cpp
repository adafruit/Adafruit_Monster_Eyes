/**
 * @file eye_support.cpp
 * @brief BMP loading, config parsing, storage, table generation and media.
 *
 * Sections, in order:
 ...
 *   5. Media        -- ties files to the renderer, with solid-colour fallback
 *
...
static void seedVariants(void) {
  for (int e = 0; e < NUM_EYES; e++) {
    ...
    v.irisSpin = settings.irisSpin;
    v.scleraSpin = settings.scleraSpin;
    v.irisStartAngle = settings.irisStartAngle;
    v.scleraStartAngle = settings.scleraStartAngle;
    v.irisMirror = settings.irisMirror;
...
static void applyObject(JsonVariantConst o) {
  ...
  settings.slitPupilRadius =
      dwim(o["slitPupilRadius"], settings.slitPupilRadius);
  settings.gazeMax = (uint32_t)dwim(o["gazeMax"], (int32_t)settings.gazeMax);

  v = o["coverage"];
  if (v.is<float>() || v.is<int>()) {
    settings.coverage = v.as<float>();
  ...
  v = o["irisAngle"];
  if (v.is<int>())
    settings.irisStartAngle = 1023 - (v.as<int>() & 1023);
  else if (v.is<float>())
    settings.irisStartAngle = 1023 - ((int)(v.as<float>() * 1024.0f) & 1023);
  v = o["scleraAngle"];
  if (v.is<int>())
    settings.scleraStartAngle = 1023 - (v.as<int>() & 1023);
  else if (v.is<float>())
    settings.scleraStartAngle = 1023 - ((int)(v.as<float>() * 1024.0f) & 1023);

  v = o["irisMirror"];
  if (v.is<bool>() || v.is<int>())
    settings.irisMirror = v.as<bool>() ? 1023 : 0;
  v = o["scleraMirror"];
  if (v.is<bool>() || v.is<int>())
...
bool eyeSettingsLoad(const char *filename) {
  ...
  seedVariants();
#if NUM_EYES > 1
  applyVariant(doc["right"].as<JsonVariantConst>(), eyeVariant[0]);
  applyVariant(doc["left"].as<JsonVariantConst>(), eyeVariant[1]);
#endif
  DBG("Loaded %s\n", filename);
  return true;
}
...
void eyeSettingsFinalize(void) {
  ...
  if (settings.eyeRadius <= 0)
    settings.eyeRadius = settings.displaySize / 2 + 5;
  else
    settings.eyeRadius = abs(settings.eyeRadius);
  ...
  if (settings.irisRadius <= 0)
    settings.irisRadius = (int)(0.4583f * (float)settings.displaySize + 0.5f);
  else
    settings.irisRadius = abs(settings.irisRadius);
  ...
  if (settings.slitPupilRadius < 0)
    settings.slitPupilRadius =
        (int)(0.4167f * (float)settings.displaySize + 0.5f);
  ...
  if (settings.coverage < needCoverage * 0.98f) {
    DBG("coverage %.2f too low for displaySize %d with eyeRadius %d; "
        "raising to %.2f\n",
        settings.coverage, settings.displaySize, settings.eyeRadius,
        needCoverage);
    DBG("  (better fix: set eyeRadius near %d in config.eye)\n",
        settings.displaySize / 2 + 5);
    settings.coverage = needCoverage;
  }
  if (settings.coverage < 0.05f)
    settings.coverage = 0.05f;
  ...
  if (settings.trackFactor < 0.0f)
    settings.trackFactor = 0.0f;
  else if (settings.trackFactor > 1.0f)
    settings.trackFactor = 1.0f;
}
...
// ===========================================================================
//
// Assets live on a FAT volume in the board's QSPI flash, read through
...
bool eyeStorageBegin(void) {
  ...
  DBG("Flash JEDEC ID 0x%06lX, %lu bytes\n", (unsigned long)flash.getJEDECID(),
...
void eyeStorageRunDriveMode(void) {
  ...
  DBGLN("DVI is intentionally off. Copy files, then eject.");
  ...
  if (!fsMounted)
    Serial.println("Volume not mountable -- format it from the host.");
  ...
  DBGLN("Drive exported. Rebooting automatically once writes stop.");
  ...
  for (;;) {
    if (mscWritten && ((millis() - lastWriteMillis) > 2000)) {
      DBGLN("Writes finished -- rebooting into eye mode.");
      flash.syncBlocks();
      delay(250);
      platformReboot();
    }
...
// Shared by every display backend
...
void displayPathTest(int size) {
  const uint16_t c[2] = {0xF800, 0x001F};
  ...
  for (int e = 0; e < NUM_EYES; e++) {
    ...
    displayEyeEnd(e);
    DBG("  path test: panel %d filled %s via the render path\n", e,
        e ? "BLUE" : "RED");
  }
  delay(2500);
}
...
float screen2map(int in) {
  return atan2f((float)in,
                sqrtf((float)(settings.eyeRadius * settings.eyeRadius -
                              in * in))) /
         (float)M_PI_2 * (float)mapRadius;
}

float map2screen(int in) {
  return sinf((float)in / (float)mapRadius) * (float)M_PI_2 *
         (float)settings.eyeRadius;
}
...
static void loadOneEyelid(const char *path, uint8_t *openT, uint8_t *closedT,
  ...
  if (path && path[0]) {
    ...
    DBG("  %s eyelid: %s unusable -- no eyelid\n", label, path);
  } else {
    DBG("  %s eyelid: none specified\n", label);
  }
...
static bool loadOneTexture(const char *path, const uint16_t **data, uint16_t *w,
                           uint16_t *h, uint32_t budget, uint16_t *solidStore,
                           uint16_t solidColor, const char *label) {
  if (path && path[0] && budget > 512) {
    FileBmpReader r(path);
    uint16_t *loaded = NULL;
    if (r.ok() && bmpLoadTexture(r, &loaded, w, h, budget)) {
#if DISPLAY_BIG_ENDIAN
      // Swap once here so the render loop never has to, and so the bytes are
      // already wire-ready for writePixels(bigEndian=true) or a DMA.
      for (uint32_t i = 0; i < (uint32_t)(*w) * (*h); i++)
        loaded[i] = __builtin_bswap16(loaded[i]);
#endif
      *data = loaded;
      DBG("  %s: %s -> %ux%u (%u bytes)\n", label, path, *w, *h,
          (unsigned)(*w * *h * 2));
      return true;
    }
    DBG("  %s: %s unusable -- solid color\n", label, path);
  } else if (path && path[0]) {
    DBG("  %s: no RAM for %s -- solid color\n", label, path);
  } else {
    DBG("  %s: none specified -- solid color\n", label);
  }
