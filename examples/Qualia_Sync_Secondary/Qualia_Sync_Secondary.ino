...
Eyes_SyncSecondary eyeSync(eyes); // Defaults: Wire, address 0x42, left eye
...
void setup() {
  ...
  Wire.end();
  Wire.setPins(SDA, SCL); // end() forgets them on some core versions

  eyeSync.begin();
}

void loop() {
  eyeSync.poll(); // Takes over gaze, pupil and lids while packets arrive
  eyes.animate();
